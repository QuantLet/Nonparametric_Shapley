...
M=200
results_list = mclapply(1:M, bs, mc.cores=16)

results = matrix(unlist(results_list), byrow = FALSE, ncol=200)
qs = t(results)

#check if true shapley value is in this quantile range (better after MC runs)
shap_true = shapley_popul(j=1, c(0,0), model_list = model_list_init) #-> definiere model_list_init global, 0.006480157

shap_true = 0.006480157
